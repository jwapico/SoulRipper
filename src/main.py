...
def update_db_with_all_spotify_data(sql_session, spotify_client):
    ...
    existing_liked_playlist = sql_session.query(SoulDB.Playlists).filter_by(name="SPOTIFY_LIKED_SONGS").first()
    if existing_liked_playlist is None:
        spotify_liked_playlist = SoulDB.Playlists.add_playlist(sql_session, spotify_id=None, name="SPOTIFY_LIKED_SONGS", description="User liked songs on Spotify - This playlist is generated by SoulRipper")
        sql_session.add(spotify_liked_playlist)
        sql_session.flush()
    ...
    for track_data in relevant_tracks_data:
        ...
        if existing_track_row:
            ...
            playlist_track_assoc = SoulDB.PlaylistTracks(track_id=new_track_row.id, playlist_id=None, added_at=track_data.date_liked_spotify)
            track_rows_and_data.append((new_track_row, track_data))
            
        if existing_liked_playlist:
            existing_liked_playlist.playlist_tracks.append(playlist_track_assoc)
        else:
